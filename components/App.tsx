...
export const App: React.FC = () => {
  // Directly start at AUTH with Global Server
  ...
  useEffect(() => {
    // Force Global Server
    authService.setServer('sv_global');
    checkSession();
  }, []);
        ...
        {gameState === GameState.MAIN_MENU && currentUser && (
          <div className="flex flex-col items-center justify-center min-h-screen p-4 animate-fade-in space-y-12 z-10 relative">
            ...
            <footer className="absolute bottom-4 text-cyan-800/60 text-[10px] text-center font-mono">
                RAPHAEL SYSTEM ONLINE. GLOBAL UNIFIED SERVER.<br/>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm p-4 animate-scale-in">
                    <div className="raphael-panel w-full max-w-md border-2 border-red-600 shadow-[0_0_50px_rgba(220,38,38,0.4)] p-6 text-center">
                         ...
                         <p className="text-cyan-100 mb-6 font-mono leading-relaxed">
                            Bạn đang yêu cầu xóa vĩnh viễn tài khoản <span className="text-red-400 font-bold">{currentUser.username}</span>.
                            <br/><br/>
                            Hành động này không thể hoàn tác.
                         </p>
                         <div className="flex gap-4 justify-center">
                             <Button onClick={() => setShowResetConfirm(false)} variant="secondary">HỦY BỎ</Button>
                             <Button onClick={handleResetAccount} variant="danger" className="animate-pulse">XÁC NHẬN XÓA</Button>
                         </div>
                    </div>
                </div>
            )}
            ...
            {showDeleteDataConfirm && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm p-4 animate-scale-in">
                    <div className="raphael-panel w-full max-w-md border-2 border-red-600 shadow-[0_0_50px_rgba(220,38,38,0.4)] p-6 text-center">
                         ...
                         <p className="text-cyan-100 mb-6 font-mono leading-relaxed">
                            Bạn sắp xóa toàn bộ tiến trình chơi game hiện tại.
                            <br/><br/>
                            Bạn có muốn thiết lập lại cuộc đời mới không?
                         </p>
                         <div className="flex gap-4 justify-center">
                             <Button onClick={() => setShowDeleteDataConfirm(false)} variant="secondary">KHÔNG</Button>
                             <Button onClick={handleDeleteGameData} variant="danger" className="animate-pulse">ĐỒNG Ý</Button>
                         </div>
                    </div>
                </div>
            )}
